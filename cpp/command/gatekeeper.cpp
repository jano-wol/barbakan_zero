...
int MainCmds::gatekeeper(int argc, const char* const* argv) {
  ...
  std::vector<int> indexHelp;
  for (int i = 0; i < gameRunner->getGameInitializer()->startPoses.size(); i++)
  {
    indexHelp.push_back(i);
  }
  std::random_shuffle ( indexHelp.begin(), indexHelp.end() );
  std::vector<int> gameIndices;
  for (int i = 0; i < gameRunner->getGameInitializer()->startPoses.size(); i++)
  {
    gameIndices.push_back(indexHelp[i]);
    gameIndices.push_back(indexHelp[i]);
  }
  ...
  auto gameLoop = [
    ...
    while(true) {
      ...
      bool res = netAndStuff->matchPairer->getMatchup(botSpecB, botSpecW, logger, true, 0);
      int gameIdx = netAndStuff->matchPairer->numGamesStartedSoFar - 1;
      lock.unlock();
      if(res) {
