...
// TODO someday - not sure how to make thread pool work with TensorMap. It works with Tensor, but TensorMap doesn't seem
// to have a device(...) method. #define EIGEN_USE_THREADS
...
void NeuralNet::freeLoadedModel(LoadedModel* loadedModel) { delete loadedModel; }

string NeuralNet::getModelName(const LoadedModel* loadedModel) { return loadedModel->modelDesc.name; }

int NeuralNet::getModelVersion(const LoadedModel* loadedModel) { return loadedModel->modelDesc.version; }
    ...
    for (int c = 0; c < in->dimension(0); c++) {
      ...
      for (int h = 0; h < in->dimension(2); h++) {
        for (int w = 0; w < in->dimension(1); w++) {
          ...
          // We're relying on all padded space being equal to 0 because this gpool only ever follows a BN+Activate with
          // a mask.
...
static size_t roundUpToMultiple(size_t size, size_t ofThis) { return (size + ofThis - 1) / ofThis * ofThis; }
...
struct ComputeContext
{
  ...
  ComputeContext(int nnX, int nnY) : nnXLen(nnX), nnYLen(nnY) {}
  ~ComputeContext() {}
};
...
struct ComputeHandleInternal
{
  ...
  ComputeHandleInternal(const ComputeContext* ctx) : nnXLen(ctx->nnXLen), nnYLen(ctx->nnYLen) {}
};
...
struct ScratchBuffers
{
  ...
  {
    std::function<float*(size_t)> allocateFunc = [this](size_t size) { return new float[size / sizeof(float)]; };
    std::function<void(float*)> releaseFunc = [this](float* buf) { delete[] buf; };

    allocator = new SimpleAllocator<float*>(allocateFunc, releaseFunc);
  }
  ~ScratchBuffers() { delete allocator; }

  size_t getBufSizeXY(int channels) const { return channels * batchXYBytes; }
  size_t getBufSize(int channels) const { return channels * batchBytes; }
};
...
struct ConvLayer
{
        ...
        nnYLen(nnY)
  {
    ...
    if ((convXSize == 3 && convYSize == 3) || (convXSize == 5 && convYSize == 5)) {
      ...
      winogradKernel = TensorMap<const Tensor<const SCALAR, 3>>(transWeights.data(), outChannels, inChannels,
      ...
      TENSOR4 kernel =
          TensorMap<const Tensor<const SCALAR, 4>>(desc.weights.data(), convXSize, convYSize, inChannels, outChannels);
  ...
  size_t requiredConvWorkspaceElts(size_t maxBatchSize) const
  {
    if ((convXSize == 3 && convYSize == 3) || (convXSize == 5 && convYSize == 5)) {
      ...
      size_t sizeForTransforms =
          totalChannelsRounded * maxBatchSize * numTilesY * numTilesX * inTileXSize * inTileYSize;
  ...
  void apply(ComputeHandleInternal* handle, CONSTTENSORMAP4* input, TENSORMAP4* output, float* convWorkspace,
             bool accumulate) const
  {
    ...
    if ((convXSize == 3 && convYSize == 3) || (convXSize == 5 && convYSize == 5)) {
      ...
      float* convWorkspaceIn =
          tile2 + inTileXSize * inTileYSize * roundUpToMultiple(std::max(inChannels, outChannels), 32);
      float* convWorkspaceOut = convWorkspaceIn + roundUpToMultiple(inChannels, 32) * batchSize * numTilesY *
                                                      numTilesX * inTileXSize * inTileYSize;
      TENSORMAP3 transformedInput(convWorkspaceIn, inChannels, batchSize * numTilesY * numTilesX,
                                  inTileXSize * inTileYSize);
      TENSORMAP3 transformedOutput(convWorkspaceOut, outChannels, batchSize * numTilesY * numTilesX,
                                   inTileXSize * inTileYSize);
      ...
      for (int dy = 0; dy < inTileYSize; dy++) {
        for (int dx = 0; dx < inTileXSize; dx++) {
          ...
          auto transformedInputMap = Eigen::Map<Eigen::Matrix<SCALAR, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor>>(
              (float*)transformedInput.data() + subTileIdx * batchSize * numTilesY * numTilesX * inChannels, inChannels,
          ...
          auto winogradKernelMap = Eigen::Map<Eigen::Matrix<SCALAR, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor>>(
              (float*)winogradKernel.data() + subTileIdx * outChannels * inChannels, outChannels, inChannels);
          auto transformedOutputMap =
              Eigen::Map<Eigen::Matrix<SCALAR, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor>>(
                  (float*)transformedOutput.data() + subTileIdx * batchSize * numTilesY * numTilesX * outChannels,
                  outChannels, batchSize * numTilesY * numTilesX);
...
struct BatchNormLayer
{
  ...
  BatchNormLayer(const BatchNormLayerDesc& desc, const ActivationLayerDesc& actDesc)
      : name(desc.name), activation(actDesc.activation)
  {
  ...
  void apply(CONSTTENSORMAP4* input, TENSORMAP4* output, CONSTTENSORMAP3* mask) const
  {
    for (int c = 0; c < input->dimension(0); c++) {
      ...
      if (activation == ACTIVATION_IDENTITY)
        ...
        output->chip(c, 0) =
            (*mask == 1.0f).select(x * (x.cwiseMin(20.0f).exp().log1p() + (x.cwiseMax(20.0f) - 20.0f)).tanh(), z);
      else
        assert(false);
    }
  }
...
struct ActivationLayer
{
  ...
  ActivationLayer(const ActivationLayerDesc& desc) : name(desc.name), activation(desc.activation) {}
...
struct MatMulLayer
{
  ...
  MatMulLayer(const MatMulLayerDesc& desc) : name(desc.name), inChannels(desc.inChannels), outChannels(desc.outChannels)
  {
...
struct MatBiasLayer
{
  ...
  MatBiasLayer(const MatBiasLayerDesc& desc) : name(desc.name), weights(desc.weights) {}
...
struct NormActConv
{
  ...
  NormActConv(const BatchNormLayerDesc& normDesc, const ActivationLayerDesc& actDesc, const ConvLayerDesc& convDesc,
              int nnX, int nnY)
  ...
  size_t requiredConvWorkspaceElts(size_t maxBatchSize) const { return conv.requiredConvWorkspaceElts(maxBatchSize); }

  void apply(ComputeHandleInternal* handle, TENSORMAP4* input, TENSORMAP4* inputScratch, TENSORMAP4* output,
             CONSTTENSORMAP3* mask, float* convWorkspace, bool accumulate) const
  {
...
struct ResidualBlockIntf
{
  ...
  virtual void apply(ComputeHandleInternal* handle, ScratchBuffers* scratch, TENSORMAP4* trunk,
                     TENSORMAP4* trunkScratch, CONSTTENSORMAP3* mask, const float* maskSum,
                     float* convWorkspace) const = 0;
...
struct ResidualBlock final : public ResidualBlockIntf
{
  ...
  {
    return std::max(normActConv1.requiredConvWorkspaceElts(maxBatchSize),
                    normActConv2.requiredConvWorkspaceElts(maxBatchSize));
  }

  void apply(ComputeHandleInternal* handle, ScratchBuffers* scratch, TENSORMAP4* trunk, TENSORMAP4* trunkScratch,
             CONSTTENSORMAP3* mask, const float* maskSum, float* convWorkspace) const override
  {
    (void)maskSum;
...
struct GlobalPoolingResidualBlock final : public ResidualBlockIntf
{
             ...
             CONSTTENSORMAP3* mask, const float* maskSum, float* convWorkspace) const override
  {
    ...
    TENSORMAP4 regularScratch(regularScratchBuf.buf, regularConv.outChannels, handle->nnXLen, handle->nnYLen,
                              batchSize);
...
struct BlockStack
{
  ...
  BlockStack(const std::vector<std::pair<int, unique_ptr_void>>& descBlocks, int nBlocks, int nnX, int nnY);
  ...
  void apply(ComputeHandleInternal* handle, ScratchBuffers* scratch, TENSORMAP4* trunk, TENSORMAP4* trunkScratch,
             CONSTTENSORMAP3* mask, const float* maskSum, float* convWorkspace) const;
};
...
struct NestedBottleneckResidualBlock final : public ResidualBlockIntf
{
  ...
  size_t requiredConvWorkspaceElts(size_t maxBatchSize) const override
  {
    return std::max(
        ...
        std::max(blocks.requiredConvWorkspaceElts(maxBatchSize), normActConv2.requiredConvWorkspaceElts(maxBatchSize)));
  }
...
BlockStack::BlockStack(const std::vector<std::pair<int, unique_ptr_void>>& descBlocks, int nBlocks, int nnX, int nnY)
    : numBlocks(nBlocks)
{
  for (int i = 0; i < numBlocks; ++i) {
    if (descBlocks[i].first == ORDINARY_BLOCK_KIND) {
      ...
      std::unique_ptr<GlobalPoolingResidualBlock> block =
          std::make_unique<GlobalPoolingResidualBlock>(*blockDesc, nnX, nnY);
      ...
      std::unique_ptr<NestedBottleneckResidualBlock> block =
          std::make_unique<NestedBottleneckResidualBlock>(*blockDesc, nnX, nnY);
...
void BlockStack::apply(ComputeHandleInternal* handle, ScratchBuffers* scratch, TENSORMAP4* trunk,
                       TENSORMAP4* trunkScratch, CONSTTENSORMAP3* mask, const float* maskSum,
                       float* convWorkspace) const
{
  for (auto& block : blocks) {
    block.second->apply(handle, scratch, trunk, trunkScratch, mask, maskSum, convWorkspace);
  }
}
...
struct Trunk
{
  ...
  size_t requiredConvWorkspaceElts(size_t maxBatchSize) const
  {
    return std::max(initialConv.requiredConvWorkspaceElts(maxBatchSize),
                    blocks.requiredConvWorkspaceElts(maxBatchSize));
  }

  void apply(ComputeHandleInternal* handle, ScratchBuffers* scratch, CONSTTENSORMAP4* input,
             CONSTTENSORMAP2* inputGlobal, TENSORMAP4* trunk, CONSTTENSORMAP3* mask, const float* maskSum,
             float* convWorkspace) const
  {
    int batchSize = trunk->dimension(3);
...
struct PolicyHead
{
  ...
  void apply(ComputeHandleInternal* handle, ScratchBuffers* scratch, CONSTTENSORMAP4* trunk, TENSORMAP2* policyPass,
             TENSORMAP4* policy, CONSTTENSORMAP3* mask, const float* maskSum, float* convWorkspace) const
  {
    int batchSize = trunk->dimension(3);
...
struct ValueHead
{
  ...
  void apply(ComputeHandleInternal* handle, ScratchBuffers* scratch, CONSTTENSORMAP4* trunk, TENSORMAP2* value,
             TENSORMAP2* scoreValue, TENSORMAP4* ownership, CONSTTENSORMAP3* mask, const float* maskSum,
             float* convWorkspace) const
  {
    int batchSize = trunk->dimension(3);
...
struct Model
{
  ...
  void apply(ComputeHandleInternal* handle, ScratchBuffers* scratch, CONSTTENSORMAP4* input,
             CONSTTENSORMAP2* inputGlobal, TENSORMAP4* trunkBuf,

             TENSORMAP2* policyPass, TENSORMAP4* policy,

             TENSORMAP2* value, TENSORMAP2* scoreValue, TENSORMAP4* ownership,

             TENSORMAP3* mask, float* maskSum, float* convWorkspace) const
  {
    *mask = input->chip(0, 0);
    computeMaskSum(mask, maskSum);

    trunk.apply(handle, scratch, input, inputGlobal, trunkBuf, mask, maskSum, convWorkspace);
    policyHead.apply(handle, scratch, trunkBuf, policyPass, policy, mask, maskSum, convWorkspace);
    valueHead.apply(handle, scratch, trunkBuf, value, scoreValue, ownership, mask, maskSum, convWorkspace);
  }
};
...
struct Buffers
{
  ...
  Buffers(const ModelDesc& desc, const Model& m, int maxBatchSize, int nnXLen, int nnYLen)
...
void NeuralNet::freeInputBuffers(InputBuffers* inputBuffers) { delete inputBuffers; }
...
ComputeContext* NeuralNet::createComputeContext(const std::vector<int>& gpuIdxs, Logger* logger, int nnXLen, int nnYLen,
                                                const string& openCLTunerFile, const string& homeDataDirOverride,
                                                bool openCLReTunePerBoardSize, enabled_t useFP16Mode,
                                                enabled_t useNHWCMode, const LoadedModel* loadedModel)
{
  (void)gpuIdxs;
  (void)logger;
...
void NeuralNet::freeComputeContext(ComputeContext* computeContext) { delete computeContext; }
...
struct ComputeHandle
{
  ...
  ComputeHandle(const ComputeContext* ctx, const LoadedModel& loadedModel, int maxBatchSize, bool iNHWC)
      : context(ctx), inputsUseNHWC(iNHWC), handleInternal(ctx), model(loadedModel.modelDesc, ctx->nnXLen, ctx->nnYLen)
  {
...
ComputeHandle* NeuralNet::createComputeHandle(ComputeContext* context, const LoadedModel* loadedModel, Logger* logger,
                                              int maxBatchSize, bool requireExactNNLen, bool inputsUseNHWC,
                                              int gpuIdxForThisThread, int serverThreadIdx)
{
  if (logger != NULL) {
    logger->write("Eigen (CPU) backend thread " + Global::intToString(serverThreadIdx) + ": Model version " +
                  Global::intToString(loadedModel->modelDesc.version));
    logger->write("Eigen (CPU) backend thread " + Global::intToString(serverThreadIdx) +
...
void NeuralNet::freeComputeHandle(ComputeHandle* gpuHandle) { delete gpuHandle; }
...
void NeuralNet::getOutput(ComputeHandle* computeHandle, InputBuffers* inputBuffers, int numBatchEltsFilled,
                          NNResultBuf** inputBufs, vector<NNOutput*>& outputs)
{
  ...
  for (int nIdx = 0; nIdx < batchSize; nIdx++) {
    ...
    SymmetryHelpers::copyInputsWithSymmetry(rowSpatial, rowSpatialInput, 1, nnYLen, nnXLen, numSpatialFeatures,
...
#define MAP4(NAME)                                                                           \
  TENSORMAP4 NAME(buffers.NAME.data(), buffers.NAME.dimension(0), buffers.NAME.dimension(1), \
                  buffers.NAME.dimension(2), batchSize)
...
#define MAP2(NAME) TENSORMAP2 NAME(buffers.NAME.data(), buffers.NAME.dimension(0), batchSize)
  ...
  computeHandle->model.apply(&computeHandle->handleInternal, computeHandle->scratch.get(), &input, &inputGlobal, &trunk,
                             &policyPass, &policy, &value, &scoreValue, &ownership, &mask, maskSum.data(),
                             convWorkspace.data());
  ...
  for (int row = 0; row < batchSize; row++) {
    ...
    if (policyChannels == 2) {
      ...
      SymmetryHelpers::copyOutputsWithSymmetry(policyProbsTmp, policyProbs, 1, nnYLen, nnXLen,
...
// FOR TESTING ---------------------------------------------------------------------------------------------------------
bool NeuralNet::testEvaluateConv(const ConvLayerDesc* desc, int batchSize, int nnXLen, int nnYLen, bool useFP16,
                                 bool useNHWC, const std::vector<float>& inputBuffer, std::vector<float>& outputBuffer)
{
  if (!useNHWC || useFP16)
    return false;
  ConvLayer layer(*desc, nnXLen, nnYLen);
  TENSORMAP4 inTensor((float*)inputBuffer.data(), desc->inChannels, nnXLen, nnYLen, batchSize);
...
// Mask should be in 'NHW' format (no "C" channel).
bool NeuralNet::testEvaluateBatchNorm(const BatchNormLayerDesc* desc, int batchSize, int nnXLen, int nnYLen,
...
bool NeuralNet::testEvaluateResidualBlock(const ResidualBlockDesc* desc, int batchSize, int nnXLen, int nnYLen,
                                          bool useFP16, bool useNHWC, const std::vector<float>& inputBuffer,
                                          const std::vector<float>& maskBuffer, std::vector<float>& outputBuffer)
{
  if (!useNHWC || useFP16)
  ...
  block.apply(&handle, &scratch, &trunk, &trunkScratch, &mask, NULL, convWorkspace.data());
...
bool NeuralNet::testEvaluateGlobalPoolingResidualBlock(const GlobalPoolingResidualBlockDesc* desc, int batchSize,
                                                       int nnXLen, int nnYLen, bool useFP16, bool useNHWC,
                                                       ...
                                                       std::vector<float>& outputBuffer)
{
  ...
  ComputeHandleInternal handle(&ctx);
  ScratchBuffers scratch(batchSize, nnXLen, nnYLen);
  block.apply(&handle, &scratch, &trunk, &trunkScratch, &mask, maskSum.data(), convWorkspace.data());
