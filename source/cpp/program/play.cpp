...
void GameInitializer::createGame(
  ...
  createGameSharedUnsynchronized(board,pla,hist,extraBlackAndKomi,initialPosition,playSettings,otherGameProps,startPosSample, forGateKeeper, gameNumber);
...
)
{
  ...
  pair<int,int> matchup; 
  if (forGateKeeper)
  {
    if ((numGamesStartedSoFar % 2) == 0)
    {
      matchup = make_pair(0,1); 
    } 
    else
    {
      matchup = make_pair(1,0);
    }  
  }
  else
  {
    matchup = getMatchupPairUnsynchronized();
...
static bool extractPolicyTarget(
  ...
  if (success == false)
  { 
    std::cerr << "problem in extractPolicyTarget\n";
    return false;
  }
...
static void recordTreePositionsRec(
  ...
  if(plaAlwaysBest && node != toMoveBot->rootNode) {
    ...
    bool suc1 = extractPolicyTarget(sp->policyTarget, toMoveBot, node, locsBuf, playSelectionValuesBuf);
    if (suc1 == false)
    {
      return;
    }
...
FinishedGameData* Play::runGame(
  ...
  for(int i = 0; i<maxMovesPerGame; i++) {
    ...
    if(!recordFullData) {
      ...
      int64_t unreducedNumVisits = toMoveBot->getRootVisits();
      bool suc2 = extractPolicyTarget(*policyTarget, toMoveBot, toMoveBot->rootNode, locsBuf, playSelectionValuesBuf);
      if (suc2 == false)
      {
        continue;
      }
  ...
  if(recordFullData) {
    ...
    if(hist.isGameFinished && hist.isNoResult) {
      ...
      finalValueTargets.score = (float)0.0;
    ...
    for(int i = 0; i<sidePositionsToSearch.size(); i++) {
      ...
      bool suc3 = extractPolicyTarget(sp->policyTarget, toMoveBot, toMoveBot->rootNode, locsBuf, playSelectionValuesBuf);
      if (suc3 == false)
      {
        continue;
      }
...
void Play::maybeForkGame(
  ...
  for(int i = 0; i<numChoices; i++) {
    ...
    bot->nnEvaluator->evaluate(copy,copyHist,getOpp(pla),nnInputParams,buf,false);
    std::shared_ptr<NNOutput> nnOutput = std::move(buf.result);
...
void Play::maybeSekiForkGame(
  ...
  if(endHist.isGameFinished && hasUnownedSpot(finishedGameData)) {
...
FinishedGameData* GameRunner::runGame(
  ...
  std::function<NNEvaluator*()> checkForNewNNEval,
  std::function<void(const Board&, const BoardHistory&, Player, Loc, const std::vector<double>&, const std::vector<double>&, const std::vector<double>&, const Search*)> onEachMove,
  bool alwaysIncludeOwnership, bool forGateKeeper, int gameNumber
) {
  ...
  if(playSettings.forSelfPlay) {
    ...
    gameInit->createGame(board,pla,hist,extraBlackAndKomi,params,initialPosition,playSettings,otherGameProps,startPosSample, forGateKeeper, gameNumber);
    botSpecB.baseParams = params;
    botSpecW.baseParams = params;
  }
  else {
    gameInit->createGame(board,pla,hist,extraBlackAndKomi,initialPosition,playSettings,otherGameProps,startPosSample, forGateKeeper, gameNumber);
