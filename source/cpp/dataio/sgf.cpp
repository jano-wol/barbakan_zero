...
void SgfNode::accumMoves(vector<Move>& moves, int xSize, int ySize) const {
  if(move.pla == C_BLACK) {
    if((move.x == COORD_MAX && move.y == COORD_MAX) ||
      ...
      moves.push_back(Move(Board::NULL_LOC,move.pla));
    else {
...
void Sgf::iterAllUniquePositionsHelper(
  ...
  samplePositionIfUniqueHelper(board,hist,nextPla,sampleBuf,initialTurnNumber,uniqueHashes,hashComments,"",f);
}
...
string WriteSgf::gameResultNoSgfTag(const BoardHistory& hist) {
  if(!hist.isGameFinished)
    ...
    return "B+" + Global::doubleToString(0);
  else if(hist.winner == C_WHITE)
    return "W+" + Global::doubleToString(0);
  else if(hist.winner == C_EMPTY)
    return "0";
  else
    ASSERT_UNREACHABLE;
