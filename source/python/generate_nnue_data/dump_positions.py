...
def main(rank: int, world_size: int, args, multi_gpu_device_ids, readpipes, writepipes, barrier):
    ...
    def maybe_reload_training_data():
        ...
        while True:
            ...
            if curdatadir != last_curdatadir:
                ...
                tdatadir = os.path.join(curdatadir,"")
    ...
    def print_board(player_stones, waiter_stones):
        ...
        for idx in range(o_num_stones):
            t = (o_s[0][idx].item(), o_s[1][idx].item())
            board_dict[t]='O'
        for idx in range(x_num_stones):
            t = (x_s[0][idx].item(), x_s[1][idx].item())
            board_dict[t]='X'
        str=""
        for idx1 in range(board_size):
            for idx2 in range(board_size):
                str += board_dict[(idx1, idx2)]
                str += " "
            str+="\n"
        str += "---------------------------------------\n"
    ...
    while True:
        ...
        for i in range(sub_epochs):
            ...
            for batch in data_processing_pytorch.read_npz_training_data(
                ...
                #for idx in range(25):
                print(f'idx={idx}')
                player_stones=batch["binaryInputNCHW"][idx][1]
                waiter_stones=batch["binaryInputNCHW"][idx][2]
                print_board(player_stones, waiter_stones)
                target_policy=postprocessed[0][0][idx] # SHOULD BE CHECKED THAT THESE ARE THE GOOD TARGETS
                target_value=postprocessed[0][1][idx] # SHOULD BE CHECKED THAT THESE ARE THE GOOD TARGETS
                #printToFile
